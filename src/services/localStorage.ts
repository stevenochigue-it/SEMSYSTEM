...
const SEED_USERS: User[] = [
  { id: '1', username: 'admin', password_hash: 'admin123', role: 'admin', full_name: 'System Administrator', active: true, created_at: '2026-01-01' },
];
...
export function googleAuthMock(googleId: string, googleEmail: string, fullName: string): User {
  ...
  // Find by google_id or google_email
  const existingIdx = users.findIndex(u => u.google_id === googleId || u.google_email === googleEmail);
  ...
  if (!users[existingIdx].google_id) {
    users[existingIdx].google_id = googleId;
    users[existingIdx].google_email = googleEmail;
    localStorage.setItem(KEYS.users, JSON.stringify(users));
  }
...
export function linkChildMock(userId: string, studentNumber: string): { success: boolean; message: string } {
  const student = getStudentByNumber(studentNumber);
  if (!student) {
    return { success: false, message: 'Student not found. Please check student number.' };
  }
  
  const users = getUsers();
  const idx = users.findIndex(u => u.id === userId);
  if (idx === -1) {
    return { success: false, message: 'Parent user account not found.' };
  }
  
  users[idx] = {
    ...users[idx],
    linked_student_number: studentNumber
  } as any;
  
  localStorage.setItem(KEYS.users, JSON.stringify(users));
  return { success: true, message: `Successfully linked to ${student.first_name} ${student.last_name}.` };
}

export function getChildStatusMock(userId: string): any {
  const users = getUsers();
  const parentUser = users.find(u => u.id === userId);
  if (!parentUser) {
    return { linked: false, message: 'Parent user not found.' };
  }
  
  const studentNumber = (parentUser as any).linked_student_number;
  if (!studentNumber) {
    return { linked: false, message: 'No student linked.' };
  }
  
  const student = getStudentByNumber(studentNumber);
  if (!student) {
    return { linked: false, message: 'Linked student record not found.' };
  }
  
  const attendance = getAttendance();
  const childLogs = attendance
    .filter(r => r.student_number === studentNumber)
    .sort((a, b) => new Date(`${b.date} ${b.time_in || '00:00'}`).getTime() - new Date(`${a.date} ${a.time_in || '00:00'}`).getTime());
