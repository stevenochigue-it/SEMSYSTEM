...
export const StudentTable: React.FC<StudentTableProps> = ({
  ...
  return (
    <Table headers={['Photo', 'Student ID', 'Full Name', 'Grade & Section', 'Status', 'Actions']}>
      {students.length > 0 ? (
        students.map((student) => {
          const sKey = student.student_id ?? student.id ?? String(Math.random());
          const displayId = student.student_id_number || student.student_number || student.student_id || student.id;
          return (
            <TableRow key={sKey}>
              {/* Photo */}
              <TableCell>
